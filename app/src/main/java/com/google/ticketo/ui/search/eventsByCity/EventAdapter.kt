...
class EventAdapter(val context: Context, val callback: EventsByCityCallback) :
    ...
    var events: List<Pair<Event, EventIntents>>? = null
    ...
    class EventHolder(val view: View, val callback : EventsByCityCallback) : RecyclerView.ViewHolder(view) {

        fun bind(data : Pair<Event, EventIntents>){

            view.item_event_by_city_image.let {
                Glide.with(it.context)
                    .load(data.first.imageUrl)
                    .apply(RequestOptions.centerCropTransform())
                    .into(it)
            }

            view.item_event_by_city_name.text=data.first.name
            val dateFormat = SimpleDateFormat("E dd.MM.yyyy")
            val timeFormat = SimpleDateFormat("HH:mm")
            view.item_event_by_city_date.text = dateFormat.format(data.first.startDate)
            view.item_event_by_city_time.text = timeFormat.format(data.first.startDate)
        }
